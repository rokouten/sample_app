...
class MicropostSidebarTest < MicropostsInterface
  ...
  test "should use proper pluralization for one micropost" do
    user = users(:malory)
    user.microposts.create!(content: "Just one micropost")
    log_in_as(user)
    get root_path
...
class ImageUploadTest < MicropostsInterface
  ...
  test "should be able to attach an image" do
    ...
    img  = fixture_file_upload("kitten.jpg", "image/jpeg", :binary)
    post microposts_path, params: { micropost: { content: cont,
                                                 image: img } }
